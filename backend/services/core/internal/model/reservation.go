...
func (r Reservation) GetAllSQL(db *sql.DB, c context.Context, reservations *[]ReservationDetails) error {
	 rows, err := db.QueryContext(
		  c,
		  "SELECT * FROM reservations LEFT JOIN users ON users.id = reservations.user_id LEFT JOIN spots ON spots.id = reservations.spot_id",
	 )
	 defer rows.Close()
	 ...
	 for rows.Next() {
		  ...
		  if err := rows.Scan(
				...
				&reservation.ReservedTo, &user.ID, &user.Email, &user.Password, &user.Role, &user.Name, &user.Surname,
	 ...
	 if len(*reservations) == 0 {
		  return response.ErrReservationsNotFound
	 }
...
func (r Reservation) DeleteSQL(db *sql.DB, c context.Context, reservation *Reservation) error {
	 rows, err := db.QueryContext(c, "DELETE FROM reservations WHERE id = ?", reservation.ID)
	 defer rows.Close()
